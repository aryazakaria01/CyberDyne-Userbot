...
if [ -z ${1} ] || [ ${1} == "-x" ] ; then
	...
	cd ${BOTDIR}
	. venv/bin/activate
	nohup python3 -m userbot >>${UNIXTIME}-cyberdyneproject.log 2>&1 & echo "I: Userbot PID is ${!}"
	deactivate
	cd ${RUNDIR}
	exit 0
elif [ ${1} == "-h" ] || [ ${1} == "--help" ] ; then
	echo -e "CyberDyne-Userbot"
	echo -e  "Licensed under Raphielscape Public License"
	echo -e "\nUsage:"
	echo -e "[no arguments] || -x -- Run in the foreground, stdout is the logcat"
	echo -e "--daemonize    || -d -- Run in the background, logs will be saved in *-cyberdyneproject.log"
	exit 0
else
	echo -e  "CyberDyne-Userbot"
        echo -e  "Licensed under Raphielscape Public License"
